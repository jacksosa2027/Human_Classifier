...
===========================
Run after prepare_datasets.py to sanity-check the unified dataset:
  • Counts images / labels per split
  • Verifies every image has a matching label and vice-versa
  • Checks label values are in [0, 1]
  • Optionally renders a grid of random samples with boxes

Usage:
    python validate_unified_dataset.py --data unified_dataset
    python validate_unified_dataset.py --data unified_dataset --visualise --n 16
"""
...
COLORS = [(0, 255, 80), (255, 80, 0), (80, 0, 255)]
...
def draw_boxes(img: np.ndarray, label_path: Path) -> np.ndarray:
    ...
    for line in label_path.read_text().splitlines():
        ...
        x2 = int((cx + bw / 2) * w)
        y2 = int((cy + bh / 2) * h)
        color = COLORS[int(parts[0]) % len(COLORS)]
...
def main():
    ...
    parser.add_argument("--data",       default="unified_dataset")
    parser.add_argument("--visualise",  action="store_true")
    parser.add_argument("--n",          type=int, default=16,
                        help="Number of samples for grid visualisation")
    args = parser.parse_args()

    base = Path(args.data)
    if not base.exists():
        print(f"Error: dataset path '{base}' does not exist.")
        raise SystemExit(1)
    print(f"\n{'═'*55}")
    print(f"  Dataset Validation: {base.resolve()}")
    print(f"{'═'*55}")
    ...
    for split in ("train", "val", "test"):
        ...
        if stats["empty_labels"]:
            print(f"  ⚠  Empty labels  ({stats['empty_labels_total']} total, first 5): {stats['empty_labels']}")
        if stats["bad_values"]:
            print(f"  ⚠  Bad values    ({stats['bad_values_total']} total, first 5): {stats['bad_values']}")
        if not any([stats["missing_label"], stats["missing_image"],
                    stats["empty_labels"], stats["bad_values"]]):
